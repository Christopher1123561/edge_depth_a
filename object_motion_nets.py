...
# from eca_net import eca_block
from cbam_net import cbam_block
...
def motion_field_net(images,
  ...
  with tf.variable_scope('MotionFieldNet'):
    with arg_scope([layers.conv2d],
      ...
      # print(images.shape) # (16, 128, 416, 8)
      ...
      # conv_input = cbam_block(conv_input)
      # padding1 = tf.keras.layers.ZeroPadding2D(padding=((3, 3), (1, 1)))
      # conv_tz1 = padding1(conv_input)
      # conv_tz1 = layers.conv2d(conv_tz1, conv_input.shape[-1], [7, 3], stride=1, padding="valid")
      # padding2 = tf.keras.layers.ZeroPadding2D(padding=((1, 1), (3, 3)))
      # conv_tz2 = padding2(conv_input)
      # conv_tz2 = layers.conv2d(conv_tz2, conv_input.shape[-1], [3, 7], stride=1, padding="valid")
      # conv_tz_add = conv_tz1 + conv_tz2
      residual_translation = _refine_motion_field(
...
def _refine_motion_field(motion_field, layer, align_corners, scope=None):
  ...
  with tf.variable_scope(scope):
    ...
    #     conv_tz_add, max(4, layer.shape.as_list()[-1]), [3, 3], stride=1)
